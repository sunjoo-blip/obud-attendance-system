...
export async function POST(req) {
  try {
    ...
    const { startDate, endDate, leaveType, startTime, endTime } =
      await req.json();
    ...
    if (leaveType === "QUARTER_DAY") {
      if (!startTime || !endTime) {
        return Response.json(
          { error: "반반차는 시간을 선택해야 합니다." },
          { status: 400 },
        );
      }
      if (daysDiff > 1) {
        return Response.json(
          { error: "반반차는 하루만 선택 가능합니다." },
          { status: 400 },
        );
      }
    }
    ...
    const balanceResult = await query(
      `SELECT total_leaves, used_leaves, COALESCE(birthday_leaves, 0) as birthday_leaves FROM leave_balance WHERE user_id = $1`,
      [session.user.id],
    );
    ...
    if (duplicateCheck.rows.length > 0) {
      for (const existing of duplicateCheck.rows) {
        ...
        // 반반차 중복 방지 (하루에 하나만)
        if (
          existing.leave_type === "QUARTER_DAY" &&
          leaveType === "QUARTER_DAY"
        ) {
          return Response.json(
            { error: "하루에 반반차는 하나만 신청 가능합니다." },
            { status: 400 },
          );
        }
      }
    ...
    // 생일 연차 먼저 차감, 부족분은 일반 연차에서 차감
    const { birthday_leaves } = balanceResult.rows[0];
    const birthdayDeduct = Math.min(leaveAmount, parseFloat(birthday_leaves));
    const normalDeduct = leaveAmount - birthdayDeduct;

    // 연차 신청 생성
    const leaveResult = await query(
      `INSERT INTO leave_requests (user_id, start_date, end_date, leave_type, start_time, end_time, status, birthday_used)
    ...
    await query(
      `UPDATE leave_balance
       ...
       WHERE user_id = $3`,
      [birthdayDeduct, normalDeduct, session.user.id],
    );
